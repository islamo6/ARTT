...
Rem Version - 1.2 (This version has the implementation of CE real time run and CSID validations)
Rem Previous Version - 1.0 (This version has the implementation of TC events translator and DMLs insertion to database).
Rem Version Creation Date: 9/30/2018
Rem Version Reviewer: Mohammad Sarwar
Rem ##########################################################################################################################################################

Rem fileLoc is the location of the config file for this tool to run.
...
Dim dbConnGbl,currUserGbl,logFileDirGbl,dbConnStrGbl,runProductGbl,systemNameGbl,runCEGbl,wsUrlGbl,excelLogDirGbl,authUserGbl,memberInfoType

Call create_excel_application_object (objXl)'Call function to create an instance of excel before the driver scripts is run.

loadTdExcelFile = load_specified_excel (tdExcelFileLoc,objXl,1,bookXl,objXlSheet, True)
...
For n= 1 To tdExcelRows
	...
	If UCase(executeFlag) = "YES" Or UCase(executeFlag) = "Y" Then 'C.1 - If any row has value "YES" for the 'EXECUTE', then read the remaining column values
		...
		If (CBool (runCERealTime) = True) Or (IsEmpty (runCERealTime) = True) Then 'C.1.2 - If flag is set to TRUE/Empty to run CE real time for all test cases for a given rule.
			runCEGbl = True
			Else
			runCEGbl = False
		End If
		...
		If InStr(memberSet,"MEM")>=0 And IsEmpty(memberSet) = False Then 'C.1.1 - If the column has this string ("MEM")
			useTdMember = True
			If InStr(memberSet,",")>=0 Then 
				memberSetArr = Split (memberSet,",")
				ElseIf InStr(memberSet,"-")>=0 Then 'C.1.1.1 - 
				memberSetArr = Split (memberSet,"-")
				memberSetFirst = memberSetArr (0)
				memberSetLast = memberSetArr (1)			
			End If 'C.1.1.1
		End If 'C.1.1
		...
		'outputExcelHeader = ruleID&"/"&ruleCat&"/"&ruleID&"/"&ruleID&"/"&ruleCat&"/"&ruleID&"/"&ruleID&"/"&ruleCat&"/"&ruleID&"/"
...
'				MsgBox fileAlreadyExists
				If fileAlreadyExists <> True Then 'C.b.2 - If the output file is not created already, then create with header.
					...
					End If 'C.b.1.a		
	'				Create Excel output file for this rule-ID.
					Call create_excel_output_file (exelObject,excelBook,excelSheet)			
			...
			If CBool(createMemberInfoExcel) = True Then 'C.c1 - If the flag is set in config file to 'True, then create the member info output excel
				memberInfoExcelGbl = "MEMBER_INFO_"&ruleID&"_"&ruleCat&"_"&Replace(Date,"/","_")&Space(1)& Replace(Time,":","_")&".xlsx"
...
'				MsgBox fileAlreadyExists
				If fileAlreadyExists <> True Then 'C.b.2 - If the output file is not created already, then create with header.
					If CStr(UCase(memberInfoType)) = "LONG" Then 'C.b.1.a - If the excel ouput is a type of new (modified from input)
						columnStr = "SUPPLIERID|MEMBERID|MEMBERPLANID|SOURCEPATIENTID|DTOFBIRTH|GENDER|PERSONID|MEMBERTYPECODE"
						ElseIf CStr(UCase(memberInfoType)) = "SHORT" Then 'C.b.1.b - If the excel ouput is a copy of input file 
						columnStr = "SUPPLIERID|MEMBERID|MEMBERPLANID"
					End If 'C.b.1.a		
	'				Create Excel output file for this rule-ID.
					Call create_excel_output_file (exelObject,excelBook,excelSheet)			
			...
			'Call function to establish database connection			
			If dbInfo = True Then 'C.a1 - If DB info was successfully retrieved from the given excel location.
				...
				If InStr(dbPassword,"|") >0 Then 'C.b1 - If the auth-key is provided with the decrypted password (ie, abcd123|MS40405
					...
					If userAuthResult = False Then 'C.1.2.1.1 - If the user authentication failed, then abort the execution.
						...
						If connectionSuccess = 0 Then 'C.1.2.1.1.1
			...
			If useTdMember = True Then 'C.a - If memberSet is specified in TD.
				...
				For d=1 To numOfMemSets
					...
					If InStr(currMemSet,",") Then 'C.b - If the current memberset has more than 1 member (separated by delimeter)
						...
						appendText = "ARTT will execute TCs (range):"&tcExecuteRange&" with "&currVarName&" needing "&totalMemInCurrSet&" new members which will be created."
						Call append_text_to_notepad_file (notePadLogDir, "",appendText)
						If createMemberInfoExcel = True Then 'C.c-If this flag (createMemberInfoExcel) in Config file is True, then create an excel file which will have member information
						End If 'C.c
						memSetHasMembers = False
					End If 'C.b
					
					For c=0 To totalMemInCurrSet 'totalMemInCurrSet = totalMemInCurrSet+1
						If memSetHasMembers = True Then 'C.c-If the current member set has list of members
							memberFromSet = Trim(currMemSetArr(c))
							Else
							memberFromSet = ""
						End If 'C.c
						myVal = execute_test_cases_for_a_given_rule_id (ruleCat,ruleID,testEnv,tcFileLoc,tcExecuteRange,memberFromSet,memSetHasMembers,notePadLogDir,excelLogDirGbl,currXlOutputFile)
					Next
					c=0'Reset the value of c
				Next 
				Else 'C.a - If member is not in memberSet
				allTcsExecuted = execute_test_cases_for_a_given_rule_id (ruleCat,ruleID,testEnv,tcFileLoc,tcExecuteRange,memberFromSet,memSetHasMembers,notePadLogDir,excelLogDirGbl,currXlOutputFile)				
			End If 'C.a
...
Rem ,tcDetailedLog (Notepad log directory),tcExcelOutput (exceloutput log directory).
...
Rem ===================================+++++++++++++++++++++++++++++++++++++++++++++++++++++++++=========================================================
Function execute_test_cases_for_a_given_rule_id (ruleCat,ruleId,testEnv,tcFileLoc,tcRange,memberFromTd,memberSetHasMember,tcDetailedLog,tcExcelOutputDir,tcExcelOutputSheetName)
	...
	Call find_test_case_range (memberFromTd,memberSetHasMember,memberIDToUse,tcRange,tcLowerLim,tcUpperLim,randomTcSelection,randSelectSingleTC)
	...
	End If 'C.e		
		...
		'call fucntion to load excel with TCs	
		loadTCExcel = load_specified_excel_by_sheet_name (tcFileLoc,objXl,ruleId,bookXl,objTCXlSheet, False)
		...
		For k = tcLowerLim+1 To tcUpperLim+1 'Loop runs from 1 to tcExcelRows if TC range is not specified in DriverScript, else runs from the lowere range of TC range.
			...
			'Get all related columns for this TC
				...
				If	useTdMember = False Or memberSetHasMember = False Then 'C.1.a	- Meaning the member should be used from TC file
					If memberSetHasMember = False Then 'C.1.b - If the member is going to be created from (Empty) MEMBERSET, then don't use MEMBERID and MEMBER_DEMOGRAPHICS columns. 
						currTcMemberId = Empty
						Else 'C.1.b
					...
					End If 'C.1.b			
					If currTcMemberId = Empty Then 'C.3.3 - If the memberID column is empty then a member with details in 'Member_Demographics' will be used to create the member.
						...
						memberLogMsg = "existing Member, provided in TC"
					End If 'C.3.3
					Else 'C.1.a
					memberLogMsg = "existing Member, provided in TD"
				End If 'C.1.a
					...
					If createMemberInfoExcel = True Then 'C.c1 - If the member info output file is created
						...
						If fileAlreadyExists = True Then 'C.c2
							...
							If UCase(memberInfoType) = "LONG" Then 'C.c3
								strExcelInfo = rowTot+1&",1;"&tcMemInfoArr(1,0)&"|"&rowTot+1&",2;"&tcMemInfoArr(1,1)&"|"&rowTot+1&",3;"&tcMemInfoArr(1,2)&"|"&rowTot+1&",4;"&tcMemInfoArr(1,3)&"|"&rowTot+1&",5;"&tcMemInfoArr(1,4)&"|"&rowTot+1&",6;"&tcMemInfoArr(1,5)&"|"&rowTot+1&",7;"&tcMemInfoArr(1,6)&"|"&rowTot+1&",8;"&tcMemInfoArr(1,7)
								numOfCols = 8
								ElseIf UCase(memberInfoType) = "SHORT" Then 'C.c3
								strExcelInfo = rowTot+1&",1;"&tcMemInfoArr(1,0)&"|"&rowTot+1&",2;"&tcMemInfoArr(1,1)&"|"&rowTot+1&",3;"&tcMemInfoArr(1,2)
								numOfCols = 3
							End If 'C.c3
...
'				End If 'C.3.3
				...
				If InStr(currTcEvents,"NOEVENTS")>0 Or InStr(currTcEvents,"NOEVENT")>0 Or InStr(currTcEvents,"NONE")>0 Then 'C.3.5 - If the TC_EVENTS column has no events specified
					...
					ElseIf InStr(UCase(currTcEvents),"REFER")>0 Then 
					'Call function to collect events for each referred TC and build DMLs
...
'					End If 'C.g.1
'					Else
'					referTCDtlArr = Split(referTCArr(g),"~")
					
'					End If 
'					Next
					ElseIf InStr(UCase(currTcEvents),"#")>0 Then 
					...
					For y = 0 To UBound(tcEventsArr)					
						insertSuccess =  execute_dml_in_database (dbConnGbl,tcEventsDMLArr(y))
						If insertSuccess = 0 Then 'C.0
							dmlCounter = dmlCounter+1
							appendTxt = "/* "&tcEventsArr(y)&" - DML is successully executed and data is inserted in DB */"&vbCrLf&tcEventsDMLArr(y)
							Else
							appendTxt = "/* "&tcEventsArr(y)&" - DML is NOT successully executed and data is not inserted in DB */"&vbCrLf&tcEventsDMLArr(y)&vbCrLf&_
				...
				curColId = get_column_id_from_excel_matching_a_column_name (objTCXlSheet,1,"DML")
				...
				If IsEmpty(currTCDml) = False Then 'C.3.4 - If there's DML provided in the DML column of TC sheet
					...
					If InStr(currTCDml,";")>=1 Then 'C.3.4.1
						...
						tcAdditionalDMLArr (0) = Replace(currTCDml,UCase("GMemberid"),memberIDToUse)
					End If 'C.3.4.1
					
					For w=0 To UBound (tcAdditionalDMLArr)
						currTCDml = tcAdditionalDMLArr(w)
						currTCDml = Replace(UCase(currTCDml),"GMEMBERID",memberIDToUse)
						insertSuccessAdditional =  execute_dml_in_database (dbConnGbl,currTCDml)
						If insertSuccessAdditional = 0 Then 'C.z - If the DML insertion failed.
							appendTxt = "/*Additional DML is successully executed and data is inserted in DB */"&vbCrLf&tcAdditionalDMLArr(w)
							Else
							appendTxt = "/*Additional DML is NOT successully executed and data is NOT inserted in DB */"&vbCrLf&tcAdditionalDMLArr(w)&vbCrLf&_
							"The reported DB error is - "&insertSuccessAdditional&"."
						End If 'C.z
				...
				If continueCSIDVal = True Then 'C.f.1 - If CE was run and CSID validtion is expected.
					...
					If IsEmpty(expectedDF) = False And IsEmpty(actualDF) = False And UCase(mfType) = UCase(evalRuleType) And actualDF Then 'C.d
						appendTxt = "The expected DF-ID ("&expectedDF&") for MF-ID ("&mfID&"), of TYPE - "&mfType&" is evaluated and the results are followed."
						commentGbl = appendTxt
						tcStatus = "PASSED"
						Else
						ReDim dfArr (1000)
...
Function create_a_member_for_tc (memberDemo,memberSupplier,testEnv,currLogFile, ByRef aggMember)
	...
	'If the TC is not provided with any of the following required demographic information, then use the ones as default from the config file
		...
		Dim tcMemInfoArr (1,7)
		Dim agMemInfoArr (1,7)
		dbConnGbl.BeginTrans
'		dbConnGbl.Execute
		'Call function to create a member in Database.
		tcMemberID = create_a_member_in_database (memberDOB,memberFirstName,memberLastName,memberMiddleInitial,memberFullName,memberEmailAddr,memberSupplier,memberGender,memberSSN,memberType,userAuthKey,dbConnGbl,currLogFile)
		'MsgBox IsEmpty (tcMemberID)&"-"&IsNull(tcMemberID) 
		If IsEmpty (tcMemberID) = False Or IsNull(tcMemberID)= False Then 'C.4.5
...
'			Call print_member_information_to_the_log (currLogFile,"",tcMemInfoArr,"REGULAR") 'Captures the member information in the log file
		...
		If pvTurnedOn = True Then 'C.4.6 - If the PersonView flag is turned on (TRUE), then create person aggregation
			agMemberID = create_a_member_in_database (memberDOB,memberFirstName,memberLastName,memberMiddleInitial,memberFullName,memberEmailAddr,personAggSupplier,memberGender,memberSSN,memberType,userAuthKey,dbConnGbl,currLogFile)
			'MsgBox "Null="&IsNull(agMemberID) &"and Empty="& IsEmpty (agMemberID)
			If IsNull(agMemberID)= False Or IsEmpty (agMemberID) = False Then 'C.4.6.1 - create the person aggregation in '' table by calling the following function.
				aggMemCreated = True
		...
		If regMemCreated = True And CBool(pvTurnedOn) = False Then 'C.4.7 - Member data was successfully inserted in 12 ODS tables, then commit the transactions and print to the log.
			...
			memberAggregated = execute_person_aggregation_table_insert_dml (dbConnGbl,tcMemberID,agMemberID,userAuthKey)
...
'		currErrCode = execute_dml_in_database (dbConnGbl,tdmMemberSQL)
	...
	For h = 0 To 11
		...
		End If 'C.1
	Next
	If errorFreeDmlCount >= 12 Then	'C.1 - If the total number of error free DMLs is >=12, meaning all DMLs were successful, then commit the transactions and retrieve member information in the array (memberInfoArr).	
...
Rem ,currSheet (sheet that needs to be scanned,columnName (value that will be searched in this column)
...
Function get_member_info_from_database (queryWithMember,dbConnGbl,ByRef memInfoArrLocal)
	sqlQuery = "select mem.ahmsupplierid,mem.memberid,mem.primarymemberplanid,mem.sourcememberpatientid,per.dtofbirth,per.gender,mem.personid,mem.membertypecode "&_
...
Function execute_member_table_insert_dml (dbConnGbl,memberSkey,currMemberSupplier,memberPatID,partySkey,currMemberType,currMemberDOB,currArttUser,memberPlanIDSeq,ByRef memArr)
	memberIdSQL = 	"INSERT INTO ODS.MEMBER (MEMBERID, DATASOURCENM, AHMSUPPLIERID, SOURCEMEMBERPATIENTID, PERSONID, MEMBERTYPECODE, EFFECTIVESTARTDT, EFFECTIVEENDDT, RECORDINSERTDT, RECORDUPDTDT, INSERTEDBY, UPDTDBY, PRIMARYMEMBERPLANID) "&_
					" VALUES  ( "&memberSkey&" , 'CAREENGINE' , "&currMemberSupplier&" , '"&memberPatID& "' , "&partySkey&", '"&currMemberType&"' , TO_DATE('"&currMemberDOB&"','DD/MM/YYYY'), NULL, SYSDATE, SYSDATE, '"&currArttUser&"' , '"&currArttUser&"' , "&memberPlanIDSeq&")" 
...
Rem ===================================+++++++++++++++++++++++++++++++++++++++++++++++++++++++++=========================================================
Function execute_person_aggregation_table_insert_dml (dbConnection,regMember,aggMember,currUser)
	...
	"VALUES ("&aggMember&","&aggMember&",SYSDATE-1,NULL,'"&currUser&"',SYSDATE,'"&currUser&"',SYSDATE)"
	...
	"VALUES ("&aggMember&","&regMember&",SYSDATE-1,NULL,'"&currUser&"',SYSDATE,'"&currUser&"',SYSDATE)"
...
Function collect_and_translate_test_case_events_into_dmls (tcEventsStr,ByRef tcEventsArr,ByRef tcEventsDMLArr,memberID)	
	...
	Next
	ReDim Preserve tcEventsArr (totalEventsCount-1)
...
Function establish_a_database_connection (dbConnStr,ByRef dbConnLcl)
	...
	If dbConnStr <> Empty And dbConn.State <> 1 Then 'C.1 - If the detailed DB information is retrieved and there's no active DB connection, then create a db connection using ADODB connection.
		...
		If Err.Number <> 0 Then 'C.1.1 - If there's no error at DB connection
			...
			returnMsg = Err.Number
'			If dbConn.State = 1 Then 'C.1..1.1
'				dbConn.Close
'			End If 'C.1.1.1
		End If 'C.1.1
	End If 'C.1
...
Rem CreatedBy: Mohammad Sarwar 
Rem ========================================================================================================================================
Function find_test_case_range (memberFromTd,memberSetHasMember,ByRef memberIDToUse,ByRef tcRange,ByRef tcLowerLim, ByRef tcUpperLim, ByRef randomTcSelection, ByRef randSelectSingleTC)
	If CBool(memberSetHasMember) = True Then 'Trim(memberFromTd) <> Empty Or Trim(memberFromTd) <> "" Then 'C.1 - If memberset is provided in the driver script instead of TC sheet.
		memberIDToUse = memberFromTd
		useTdMember = True 'If member is passed from MemberList from Test Driver, set this to 'True'
		ElseIf CBool(memberSetHasMember) = False Then
		memberIDToUse = ""'"Null"
		useTdMember = True
'		If InStr(tcRange,"-")>0 Then  'C.a
...
Rem FunctionParams: fileDirectPath (if the folderLoc value is the direct path of the file including the extension, 
